...
uint16_t sin_tab[256] = {32768, 33572, 34375, 35178, 35979, 36779, 37575, 38369, 39160, 39947, 40729, 41507, 42279, 43046, 43807, 44560,
...
ISR( TIMER1_COMPA_vect ) {
    ...
    for ( uint16_t mask = 0x8000;mask > 0;mask = mask >> 1 ) {
        if (( sin_tab[probe_idx]&mask ) == 0 ) DAT_LO();
        else DAT_HI();
        tda1543_clk();//clk pulse
    }
    // right channel
